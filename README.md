...
The HMA demo repository is available at [2025-project-hma-clip-demo](https://github.com/UMass-Rescue/2025-project-hma-clip-demo).

To get HMA up and running, follow these steps:
...
2. **Launch the Services**
   ...
   You can configure the PostgreSQL database and other environment variables for HMA in `docker-compose.yml` and `omm_config.py`.
...
1. **Ensure the Latest HMA Version**
   ...
   In the Dockerfile of HMA, ensure the latest version of HMA is pulled. If not, update line 1 in the Dockerfile to:
...
2. **Use an External Network**
   ...
   Before running HMA, configure it to use an external network:
...
3. **Run the Evaluation**
   ...
   Ensure HMA is running, then execute the evaluation pipeline using the following command:
